...
namespace bblProg2
{
    ...
    {
        assert(fechaCorrecta(fec)); // Se verifica el formato de la fecha
        fecha = fec;
    }
    ...
    {
        string procesado, pal;
        unsigned cont_pos = 0, cont_neg = 0;
        procesarComentario(comentario, procesado);
        unsigned pos = 0;
        while (pos < procesado.size())
        {
            pal = "";
            while (pos<procesado.size() && procesado[pos] != ' ') // Falta última palabra (o una única palabra en procesado)
            {
                pal += procesado[pos];
                ++pos;
            }
            if (pal_pos.buscar(pal))
            {
                ++cont_pos;
            }
            else if (pal_neg.buscar(pal))
            {
                ++cont_neg;
            }
            ++pos;
    ...
    bblProg2::Sentimiento Valoracion::consultarSentimiento() const
    {
        return sentimiento;
    }
    ...
    void Valoracion::escribir() const
    {
        ...
        cout << "--- COMENTARIO:" << endl
             << comentario << endl;
        cout << "*** SENTIMIENTO: ";
        escribirSentimiento(sentimiento);
        cout << endl;
    }
} // namespace bblProg2
